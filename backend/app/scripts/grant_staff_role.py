...
def grant_staff_role(email: str, role: str) -> int:
    ...
    response = client.table("app_users").select("id,email,role,email_verified_at,mfa_enabled").eq("email", normalized).limit(1).execute()
    ...
    if not target.get("email_verified_at"):
        ...
        return 4
    if not target.get("mfa_enabled"):
        print("Target account must enroll and verify MFA before receiving a staff role.", file=sys.stderr)
        return 5
    ...
    client.table("user_sessions").update({"revoked_at": datetime.now(timezone.utc).isoformat()}).eq("user_id", target["id"]).is_("revoked_at", "null").execute()
