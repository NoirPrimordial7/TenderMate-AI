...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return <div className="na-console">
    <aside className="na-sidebar"><Link className="na-brand" href="/admin">NividaIQ <span>{t("console")}</span></Link><nav aria-label={t("navigation")}>{routes.map((route) => { const href = `/admin${route ? `/${route}` : ""}`; return <Link aria-current={pathname === href ? "page" : undefined} href={href} key={route || "overview"}>{t(route || "overview")}</Link>; })}</nav></aside>
