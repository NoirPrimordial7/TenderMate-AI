...
export function SignUpForm({ onAuthenticated }: { onAuthenticated?: () => void }) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    if (!fullName.trim() || !email.trim() || password.length < 6 || !acceptedLegal) {
      setError(t("signupMissing"));
      return;
    }
    setIsSubmitting(true);
    try {
      await signup({ full_name: fullName.trim(), email: email.trim(), password, preferred_language: activeLocale, preferred_analysis_language: analysisLocale, accepted_legal: acceptedLegal, legal_locale: activeLocale });
      onAuthenticated?.();
    } catch (submitError) {
